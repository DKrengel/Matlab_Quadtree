function [acx,acy,acf,overlapList] =DEM_interaction(...
  col_list,col_list_len,dt,particleX,particleY,...
  particleSide,PX,PY,VX,VY,VF,freeParticle,m,mi)
% PUPOSE: driver to call the computation of the overlap geometry and from 
%  that the interaction forces
% CAVEAT: This is the most CPU-time-consuming routine for dense systems; 
%  speeding up other functions will
%  hardly have any impact
% LITERATURE: sec. 7.2,7.3 
% REVISION HISTORY: 
%    15-May-2014 Shi Han Ng, HG Matuttis 
%    29-Jul-2015 modifications for contact histogram computation
%    2024-2025; Quadtree adaptation
...
overlapList = zeros(2, col_list_len);
overlapIndex = 0;

% Loop over the collision list which contains the particle pairs, i.e. the
% respective two indices of each of the interacting pairs
for i = 1:col_list_len
  % Reorder the indices of the interacting pairs so that the index for the 
  % first particle is smaller than for the second particle 
...
% If-condition serves to skip physically irrelevant computation of overlaps
% between walls which may be geometrically problematic
  if (freeParticle(polyA)|freeParticle(polyB))
    overlap_geometry = ...
      DEM_overlap(particleX(1:particleSide(polyA),polyA),...
    ...
    if( overlap_geometry(1) > 0 )
      % To check outputs of tree-code is right or not
      overlapIndex = overlapIndex + 1;
      overlapList(:, overlapIndex) = [polyA polyB]';
...
% Enter contact point in the contact point histogram if
% one collision partner is a particle and the other a blade
    if (((polyA==iblade1)&&(polyB>(nwall_i)))||...
        ((polyA==iblade2)&&(polyB>(nwall_i)))||...
        ((polyB==iblade2)&&(polyA>(nwall_i)))||...
        ((polyB==iblade2)&&(polyA>(nwall_i))))
    end
    
%compute the forces based on the overlap geometry
      [resulting_forces,ttanfList(realListLen),areaList(realListLen),...
      vreltList(realListLen)] = ...
      DEM_findForce(overlap_geometry,...
...
overlapList = overlapList(:, 1:overlapIndex);
