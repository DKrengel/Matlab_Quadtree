function [col_list,col_list_len] = neighborhood_algorithm(polys,polyx,polyy)
%  PURPOSE: Neighborhood algorithm via "sort and sweep"
%    1. Re-compute the bounding boxes
%    2. Update the corresponding lists A_X, AY
%    3. Permute the lists A_X, ... A_Y ..., C_Y and register possible 
%       interacting particle pairs according to the overlap of bounding boxes
% INPUT:
%  polys - Number of corners in each polygon.
%  polyx, polyy - Corners of each polygons.
%  
% OUTPUT:
%  col_list - List of possible contacting polygons. 
%  col_list_len - Number of possible contacting polygons.
% USAGE: The persistent variables in this function must be cleared at the 
%   very beginning of the main program with clear neighborhood_algorithm
% REVISION HISTORY:
...
%  Vectorizable (no bank conflicts would arise on a NEX-SX supercomputer) incremental sort bounding boxes:
%  x-sorting: take into account only new bounding boxes, like in 1D.
%  y-sorting: take into account old and new bounding boxes.
%
% CAVEAT: 
%  If the bounding boxes of some particles overlap or even TOUCH during 
%  initialization, the neighborhood algorithm may not be able to detect
%  these interacting pairs
% TODO: 
%  Write a control routine for the particle initialization so that overlaps 
%  during the initialization are detected to avoid the problem in "CAVEAT"
% LITERATURE: sec. 7.5
...
new_col_list_len=0;
new_col_list=zeros(2,8*polyn);
% Make a list of particles newly in contact, new_col_list, which is defined as
...
% Append those elements to the new collision lists from the old collision 
% lists where the bounding boxes still have an overlap
